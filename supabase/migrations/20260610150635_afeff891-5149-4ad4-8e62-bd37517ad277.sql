...
BEGIN
  ...
  -- Deterministic tie-break: when a user owns multiple workspaces (legacy/orphan
  -- duplicates), always pick the OLDEST one. Without ORDER BY, Postgres can flip
  -- between rows across sessions, causing role/workspace oscillation in AccountContext.
