...
RETURNS TABLE (
  user_id uuid, member_id uuid, full_name text, email text, avatar_url text,
  roles text[], team_id uuid, team_name text, team_count integer,
  leader_user_id uuid, leader_name text, status text, invite_status text,
  has_sync boolean, is_linked boolean, last_activity_at timestamptz, created_at timestamptz
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  v_uid uuid := auth.uid();
BEGIN
  -- Quando a sessão ainda não hidratou (auth.uid IS NULL), retornamos 0 rows
  -- silenciosamente em vez de levantar 'forbidden'. O frontend faz retry
  -- automático na queryKey [..., user.id] quando o JWT chega.
  ...
  pending AS (
    SELECT
      ...
      (tm.skills_data IS NOT NULL OR tm.job_crafting_profile IS NOT NULL) AS has_sync,
      tm.created_at
    FROM public.team_members tm
    JOIN public.teams t ON t.id = tm.team_id
    WHERE t.workspace_id = p_workspace_id
  ...
  SELECT
    ...
    CASE
      ...
      ELSE (tm.skills_data IS NOT NULL OR tm.job_crafting_profile IS NOT NULL)
    END AS has_sync,
    true AS is_linked,
    GREATEST(
