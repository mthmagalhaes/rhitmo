CREATE OR REPLACE FUNCTION public.normalize_attendees(_att jsonb)
RETURNS jsonb
LANGUAGE plpgsql
IMMUTABLE
...
DECLARE
  inner_val jsonb;
BEGIN
  IF _att IS NULL THEN RETURN '[]'::jsonb; END IF;
  IF jsonb_typeof(_att) = 'array' THEN RETURN _att; END IF;
  IF jsonb_typeof(_att) = 'string' THEN
    BEGIN
      inner_val := (_att #>> '{}')::jsonb;
    EXCEPTION WHEN others THEN
      RETURN '[]'::jsonb;
    END;
    IF jsonb_typeof(inner_val) = 'array' THEN RETURN inner_val; END IF;
  END IF;
  RETURN '[]'::jsonb;
END;
$$;

REVOKE ALL ON FUNCTION public.normalize_attendees(jsonb) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.normalize_attendees(jsonb) TO service_role;
...
BEGIN
  ...
  meeting_members AS (
    ...
    CROSS JOIN LATERAL jsonb_array_elements_text(public.normalize_attendees(um.attendees)) AS att(email)
