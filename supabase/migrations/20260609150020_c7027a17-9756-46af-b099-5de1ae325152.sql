...
-- 1) Atualizar get_account_context: aceitar liderados vinculados independentemente do invite_status
...
BEGIN
  ...
  -- FIX: aceitar liderados vinculados independentemente do invite_status.
  -- Antes exigia invite_status='accepted', o que deixava liderados criados via
  -- bulk-onboard (status='none') invisíveis e empurrava-os pra criar workspace.
...
-- 2) Backfill defensivo: liderados já vinculados mas com status legado.
UPDATE public.team_members
SET invite_status = 'accepted'
WHERE linked_user_id IS NOT NULL
  AND COALESCE(invite_status, 'none') NOT IN ('accepted', 'revoked');

-- 3) Cleanup: remover workspaces "Faster" duplicados criados hoje por liderados.
DELETE FROM public.teams
WHERE workspace_id IN (
  '0b75fe37-557a-44c4-bd99-a91b47386c28',
  'a0e9a22e-67fb-4748-b465-93454e048f24'
);
