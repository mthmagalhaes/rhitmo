CREATE OR REPLACE FUNCTION public.get_team_timeline(
  _workspace_id uuid DEFAULT NULL::uuid,
  _member_ids uuid[] DEFAULT NULL::uuid[],
  _source_tables text[] DEFAULT NULL::text[],
  _before timestamp with time zone DEFAULT NULL::timestamp with time zone,
  _limit integer DEFAULT 30
)
RETURNS TABLE(id uuid, member_id uuid, member_name text, member_avatar text, evidence_type text, source_table text, source_id uuid, occurred_at timestamp with time zone, title text, summary text, sentiment text, visibility text, metadata jsonb)
LANGUAGE plpgsql
