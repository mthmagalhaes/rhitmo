// Envia o e-mail de boas-vindas / convite para um liderado recém-cadastrado.
...
serve(async (req) => {
  ...
  try {
    ...
    const { memberId, useInviteLink } = await req.json()
    ...
    const syncUrl =
      useInviteLink && member.inviteToken
        ? `${APP_URL}/invite?code=${member.inviteToken}`
        : `${APP_URL}/sync/${member.id}`

    const result = await sendAppEmail('member-welcome', member.email, {
      idempotencyKey: `member-welcome-${member.id}-${Date.now()}`,
      templateData: {
    ...
    console.error('[send-member-invite] failed:', message)
